  ...
  <div>
    <BaseCard title="Рынок труда" number="F1" :show-open-button="true" @open="visible = true">
      ...
      <div v-else class="text-white grid grid-cols-2 gap-2">
        <div class="relative">
          <highcharts :options="chartOptions" class="h-[250px] w-full m-auto"></highcharts>
          <div class="absolute text-center top-[90px] m-auto left-1/2 -translate-x-1/2">
        ...
        <div>
          <ul>
            <li class="flex text-[10px] items-center gap-2 mb-3 text-xs justify-between" v-for="item in list"
              :key="item.title">
...
const naselenie = computed(() => data.value.reduce((acc, curr) => acc + curr.cnt, 0))
const bezrabot = computed(() => data.value.reduce((acc, curr) => acc + curr.rt_unemployed, 0))
const trudo = computed(() => data.value.reduce((acc, curr) => acc + curr.trud_vozrast, 0))
const rabot = computed(() => data.value.reduce((acc, curr) => acc + curr.working, 0))
const nezanyat = computed(() => data.value.reduce((acc, curr) => acc + curr.nezanyat, 0))
