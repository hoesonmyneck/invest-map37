<template>
    <BaseCard title="Вакансии / резюме" number="F3" @open="visible = true" :show-open-button="true">
        ...
        <div v-else class="text-white">
            <div class="flex gap-1">
                ...
                <div :class="{ 'active': tab === 1 }" @click="tab = 1" class="btn mini">Наиболее дефицитные</div>
                <div :class="{ 'active': tab === 2 }" @click="tab = 2" class="btn mini">Наиболее избыточные</div>
            </div>
            <div class="overflow-auto h-[calc(50vh-106px)]">
                <template v-if="tab === 0">
                    ...
                    <ul class="flex gap-4 flex-col text-[10px] justify-center">
                        <li class="flex gap-2">
                            <p class="w-4 h-4 rounded-full" style="background-color: #4990D3;"></p> Весь рынок: <b>{{
                                Numeral(vacancy_count) }}</b>
                        </li>
                        <li class="flex gap-2">
                            <p class="w-4 h-4 rounded-full" style="background-color: #D568FB;"></p> Квалифицированные
                            профессии: <b>{{ Numeral(qual_prof) }}</b>
                        </li>
                        <li class="flex gap-2">
                            <p class="w-4 h-4 rounded-full" style="background-color: #D15B32;"></p> Рабочие профессии:
                ...
                <template v-else>
                    <div class="list pt-5">
                        <div v-for="i in Object.values(groupByNkz).splice(0, 15)" :key="i"
                            ...
                            <div>
                                <div class="flex items-center">
                                    <a-progress class="h-2" strokeColor="#54ACFE" trailColor="#3B3F4B"
                                        :show-info="false"
                                        :percent="+Numeral((i.vacancy_count / maxVacancyCount * 100))" />
                                    <p class="text-[10px] w-[40px]">{{ i.vacancy_count }}</p>
                                </div>
                                <div class="flex items-center">
                                    <a-progress class="h-2" strokeColor="#FE6A35" trailColor="#3B3F4B"
                                        :show-info="false"
...
const qual_prof = computed(() => data.value.reduce((acc, curr) => acc += curr.qual_prof, 0))
const rab_prof = computed(() => data.value.reduce((acc, curr) => acc += curr.rab_prof, 0))
const vacancy_count = computed(() => data.value.reduce((acc, curr) => acc += curr.vacancy_count, 0))

const groupByNkz = computed(() => Object.values(data.value.reduce((acc, curr) => {
...
const maxVacancyCount = computed(() => Object.values(groupByNkz.value).sort((a, b) => b.vacancy_count - a.vacancy_count)[0]?.vacancy_count)
const maxResumeCount = computed(() => Object.values(groupByNkz.value).sort((a, b) => b.resume_count - a.resume_count)[0]?.resume_count)
