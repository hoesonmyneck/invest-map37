            ...
            <div class="grid grid-cols-[1fr_2fr] text-white">
                <div>
                    <div class="flex gap-1">
                        <div :class="{ 'active': tab === 0 }" @click="tab = 0" class="btn">Соотношение</div>
                        <div :class="{ 'active': tab === 1 }" @click="tab = 1" class="btn">Общая площадь</div>
                        <div :class="{ 'active': tab === 2 }" @click="tab = 2" class="btn">Количество голов</div>
                    </div>
                    <div class="overflow-auto h-[calc(50vh-106px)]">
                        <template v-if="tab === 0">
                            <div class="relative mt-5">
                                ...
                                <div class="absolute top-1/2 text-center left-1/2 -translate-x-1/2 -translate-y-1/2">
                                    <p class="text-2xl">{{ Numeral(allCount) }}
                                    </p>
                        ...
                        <template v-else>
                            <p v-if="tab === 1" class="mb-2 pb-2 border-b mt-4 text-sm">Общая площадь: <b>{{
                                    Numeral(Object.values(totalArea).reduce((acc, curr) => acc + curr.area, 0)) }}</b>
                                га
                            </p>
                            <p v-if="tab === 2" class="mb-2 pb-2 border-b mt-4 text-sm">Количество голов:
                ...
                <div class="ml-4 pl-4 border-l border-gray-600 overflow-scroll">
                    ...
                    <div class="overflow-y-auto h-[calc(40vh-50px)] w-full">
                        <div class="head gap-1 grid grid-cols-[100px_100px_100px_100px_100px_100px_100px_50px_50px] text-[10px] mt-1"
                            ...
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">{{
                                item.bin }}</p>
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">{{
                                +item.tip === 1 ? 'Растениеводство' : 'Животноводство' }}</p>
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">{{
                                item.crop_ru }}</p>
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">
                                {{+item.tip === 1 ? item.area : ''}}</p>
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">{{
                                +item.tip !== 1 ? item.area : '' }}</p>
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">{{
                                item.total_head_count }}</p>
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">-</p>
                            <p class="h-6 w-full  px-1 flex items-center bg-[#252A36] rounded w-full truncate">-</p>
                        </div>
                    </div>
                </div>
                ...
                <l-map ref="mapRef" :zoom="5" :center="[49.213962, 67.109173]" :options="{ zoomControl: false }"
                    ...
                    <template>
                        <l-polygon v-for="p in Object.values(regionPolygons.features)" :key="p.parent1_code"
                            ...
                            :fillColor="!!currentRegion ? (currentRegion === p.properties.parent1_code ? getColorFromGradient(groupByRegion[+p.properties.parent1_code]?.area / maxCountGroupByRegion * 100) : '#252A36') : getColorFromGradient(groupByRegion[+p.properties.parent1_code]?.area / maxCountGroupByRegion * 100)">
                            <l-tooltip class="p-0 bg-transparent rounded-md">
                                ...
                                <div class="flex items-center gap-2 ">
                                    ...
                                    <p class="font-bold">{{ Numeral(groupByRegion[+p.properties.parent1_code]?.area) }}</p>
                                </div>
                            </l-tooltip>
                        </l-polygon>
                    </template>
...
const allCount = computed(() => Object.values(_transformedData.value)?.reduce((acc, curr) => acc + curr.new_id, 0))

const _transformedData = computed(() => data.value.filter(item => (currentRegion.value === '' || item.parent1_code === currentRegion.value) && (tab.value === 0 ? true : (item.tip === (tab.value === 1 ? 'Растениеводство' : 'Животноводство')))).reduce((acc, curr) => {
...
const dataListFiltered = computed(() => dataList.value.filter(item => (currentRegion.value === '' || item.parent1_code === currentRegion.value) && (tab.value === 0 ? true : +item.tip === tab.value )))

const type_1 = computed(() => Object.values(_transformedData.value)?.filter(item => item.tip === 'Растениеводство'))
const type_2 = computed(() => Object.values(_transformedData.value)?.filter(item => item.tip === 'Животноводство'))

const totalArea = computed(() => data.value.filter(item => item.tip === (tab.value === 1 ? 'Растениеводство' : 'Животноводство'))?.reduce((acc, curr) => {
    if (['Кулан', 'Пони', 'Ослы', 'Верблюды', 'Свиньи', 'Козы', 'Мулы'].includes(curr.crop_ru)) return acc
...
const groupByRegion = computed(() => data.value.filter(item => item.tip === (tabMapStatus.value === 1 ? 'Растениеводство' : 'Животноводство'))?.reduce((acc, curr) => {
...
const maxCountGroupByRegion = computed(() => Object.values(groupByRegion.value)?.sort((a, b) => b.area - a.area)[0]?.area)
...
const chartOptions2 = computed(() => {
    return {
        ...
        xAxis: {
            categories: Object.values(totalArea.value).sort((a, b) => b.area - a.area).map(item => item.crop_ru),
        ...
        series: [{
            ...
            pointWidth: 12,
            borderWidth: 0,
            data: Object.values(totalArea.value).sort((a, b) => b.area - a.area).map((item) => {
                return {
                    name: item.crop_ru,
                    y: item.area,
