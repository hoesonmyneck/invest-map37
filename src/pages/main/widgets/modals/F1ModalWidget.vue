        ...
        <BaseCard v-else title="" number="" :show-close-button="true" @close="$emit('close')">
            <div class="grid grid-cols-2 gap-3">
                <div class="grid grid-cols-1 gap-3 text-white">
                    <div>
                        <p class="mb-2 px-4 py-1 rounded bg-[#252A36] flex items-center">«СЕРПIН»</p>
                        <div class="grid grid-cols-[200px_1fr] gap-2 items-start p-3 px-5 relative">
                            <div class="relative flex items-center justify-center">
                                <highcharts :options="chartSerpin" class="h-[170px] m-auto"></highcharts>
                                <p class="absolute text-2xl font-bold">{{ Numeral(ispolnayet / total * 100) }}%</p>
                            </div>
                            <ul class="">
                                <li
                    ...
                    <div>
                        <p class="mb-2 px-4 py-1 rounded bg-[#252A36] flex items-center">«АУЫЛ АМАНАТЫ»</p>
                        <div class="grid grid-cols-[200px_1fr] gap-2 items-start p-3 px-5">
                            <div class="relative flex items-center justify-center">
                                <highcharts :options="chartAulAmanati" class="h-[170px] m-auto"></highcharts>
                                <p class="absolute text-2xl font-bold">{{ Numeral(opv_has / total1 * 100) }}%</p>
                            </div>
                    ...
                    <div>
                        <p class="mb-2 px-4 py-1 rounded bg-[#252A36] flex items-center">«ДИПЛОММЕН АУЫЛҒА»</p>
                        <div class="grid grid-cols-[200px_1fr] gap-2 items-center p-3 px-5">
                            <div class="relative flex items-center justify-center">
                                <highcharts :options="chartDiplommenAulga" class="h-[170px] m-auto"></highcharts>
                                <p class="absolute text-2xl font-bold">{{ (rabotaet_aul / total2 * 100).toFixed(0) }}%
                                </p>
                            </div>
                            <ul>
                                <li
                                ...
                                <li v-for="(l, i) in listDiplommenAulga"
                ...
                <div class="text-white">
                    <p class="mb-2 px-4 py-1 w-full rounded bg-[#252A36] flex items-center">«АУЫЛ-ЕЛ БЕСІГІ»</p>
                    <div class="text-white overflow-x-scroll h-[calc(70vh)] w-full">
                        <div class="header-grid-text items-end w-full" style="margin-bottom: 0; padding: 0 10px;">
                        ...
                        <div class="header-grid-text3 w-full border-b border-gray-700 text-gray-400 mb-2">
                            <p>{{ Numeral(aulBesigi.length) }}</p>
                            <p>{{ Numeral(totalSumm) }}</p>
                        </div>
                        <div class="">
                            <div v-for="l in aulBesigi.sort((a, b) => b.percentage_change - a.percentage_change)"
                                :key="l.project_name + l.name + l.start_date + l.end_date + l.plan_rab + l.fact_rab"
                                class="header-grid-text3 text-xs" style="margin-bottom: 0; padding: 2px 5px;">
                                ...
                                <div
                                    ...
                                    <div class="absolute top-0 left-0 h-full rounded w-full"
                                        :style="{ background: getColorFromGradient(l.percentage_change ?? 0), width: `${l.percentage_change > 100 ? 100 : l.percentage_change ?? 0}%` }">
                                    </div>
...
import { getSerpin, getAulAmanati, getDiplommenAulga, getAulBesigi } from '../../../../entities/f/api';
...
const filterByRegion = (data: any[]) =>
    data.filter(e => (!currentRegion.value ? true : +e.parent1_code === +currentRegion.value))

const calculateTotal = (data: any[], key: string) => computed(() =>
...
const ispolnayet = computed(() => calculateTotal(serpin.value, 'ispolnayet').value);
const total = computed(() => calculateTotal(serpin.value, 'total').value);


const listSerpin = computed(() => [
    { name: 'Работающие', count: rabotaet.value, percent: rabotaet.value / total.value * 100 },
    { name: 'Безработные', count: bezrabot.value, percent: bezrabot.value / total.value * 100 },
    { name: 'С трудовым договором:', count: est_trud.value, percent: est_trud.value / total.value * 100 },
    { name: 'Без трудового договора', count: bez_trud.value, percent: bez_trud.value / total.value * 100 },
    { name: 'Исполняющие', count: ispolnayet.value, percent: ispolnayet.value / total.value * 100 },
]);
...
const active_ip = computed(() => calculateTotal(aulAmanati.value, 'active_ip').value);
const not_active = computed(() => calculateTotal(aulAmanati.value, 'not_active').value);
const opv_has = computed(() => calculateTotal(aulAmanati.value, 'opv_has').value);
const total1 = computed(() => calculateTotal(aulAmanati.value, 'total').value);

const listAulAmanati = computed(() => [
    { name: 'Срок до 5 лет', count: srok_5.value, percent: srok_5.value / total1.value * 100 },
    { name: 'Срок до 7 лет', count: srok_7.value, percent: srok_7.value / total1.value * 100 },
    { name: 'Активное ИП', count: active_ip.value, percent: active_ip.value / total1.value * 100 },
    { name: 'Неактивное ИП', count: not_active.value, percent: not_active.value / total1.value * 100 },
    { name: 'Есть ОПВ', count: opv_has.value, percent: opv_has.value / total1.value * 100 },
]);

const rabotaet_aul = computed(() => calculateTotal(diplommenAulga.value, 'rabotaet_aul').value);
const bezrabot2 = computed(() => calculateTotal(diplommenAulga.value, 'bezrabot').value);
const total2 = computed(() => calculateTotal(diplommenAulga.value, 'total').value);

const listDiplommenAulga = computed(() => [
    { name: 'Работающие в селе', count: rabotaet_aul.value, percent: rabotaet_aul.value / total2.value * 100 },
    { name: 'Безработные', count: bezrabot2.value, percent: bezrabot2.value / total2.value * 100 },
]);
...
const chartSerpin = computed(() => chartOptions(
    [
        'Общее количество',
        'Работающие',
        'Безработные',
...
const chartAulAmanati = computed(() => chartOptions(
    [
        'Общее количество',
        'Работающие',
        'Безработные',
...
const chartDiplommenAulga = computed(() => chartOptions(
    [
        'Общее количество',
        'Работающие',
        'Безработные',
