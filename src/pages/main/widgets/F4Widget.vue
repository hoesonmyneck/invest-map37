  ...
  >
    ...
    <div v-else class="text-white grid grid-cols-[200px_1fr] gap-2">
      ...
      <div class="overflow-y-scroll h-[calc(50vh-65px)]">
        <ul>
          <li
            class="flex text-[10px] items-center gap-2 mb-1 text-xs justify-between min-w-[200px]"
            v-for="item in Object.values(list).sort(
              (a, b) => b.count - a.count
            )"
            :key="item.otrasl"
          >
            <div class="flex gap-2 items-center">
              <p
                class="min-w-4 w-4 h-4 rounded-full"
            ...
            <div class="flex gap-2 items-center">
              ...
              <p class="bg-black p-1 rounded px-2 w-[40px] text-center">
                {{ Numeral((item.count / data.length) * 100) }}%
              </p>
            </div>
          </li>
        </ul>
