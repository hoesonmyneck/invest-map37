<template>
    <div class="text-white">
        <BaseCard title="«ДИПЛОММЕН АУЫЛҒА»" number="A3" :show-close-button="false" @close="$emit('close')">
            <div class="grid grid-cols-[200px_1fr] gap-2 items-center p-3 px-5">
                ...
                <ul>
                    <li class="grid grid-cols-[40px_1fr_80px_70px] w-full mb-2 pb-2 border-b border-gray-700">
...
const rabotaet_aul = computed(() => diplommenAulgaFilter.value.reduce((acc, curr) => +acc + +(curr.rabotaet_aul || 0), 0));
const bezrabot = computed(() => diplommenAulgaFilter.value.reduce((acc, curr) => +acc + +(curr.bezrabot || 0), 0));
const total = computed(() => diplommenAulgaFilter.value.reduce((acc, curr) => +acc + +(curr.total || 0), 0));
...
async function loadSerpin() {
    data.value = await getDiplommenAulga().finally(() => {
        loader.value = false;
    });

    programStore.setDiplommenAulga(data.value);
}
