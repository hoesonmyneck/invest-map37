<template>
    <div class="text-white">
        <BaseCard title="«СЕРПIН»" number="A1" :show-close-button="false" @close="$emit('close')">
...
const rabotaet = computed(() => serpinFilter.value.reduce((acc, curr) => +acc + +(curr.rabotaet || 0), 0));
const bezrabot = computed(() => serpinFilter.value.reduce((acc, curr) => +acc + +(curr.bezrabot || 0), 0));
const est_trud = computed(() => serpinFilter.value.reduce((acc, curr) => +acc + +(curr.est_trud || 0), 0));
const bez_trud = computed(() => serpinFilter.value.reduce((acc, curr) => +acc + +(curr.bez_trud || 0), 0));
const ispolnayet = computed(() => serpinFilter.value.reduce((acc, curr) => +acc + +(curr.ispolnayet || 0), 0));
const total = computed(() => serpinFilter.value.reduce((acc, curr) => +acc + +(curr.total || 0), 0));
...
async function loadSerpin() {
    data.value = await getSerpin().finally(() => {
        loader.value = false;
    });
