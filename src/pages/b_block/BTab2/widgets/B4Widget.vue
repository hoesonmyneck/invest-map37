<template>
    <div class="text-white">
        <BaseCard title="«АУЫЛ-ЕЛ БЕСІГІ»" number="A4" :show-close-button="false" @close="$emit('close')">
            <div class="text-white overflow-x-scroll h-[calc(20vh)] w-full">
                <div class="header-grid-text items-end text-xs w-full" style="margin-bottom: 0; padding: 0 10px">
                ...
                <div class="">
                    <div v-for="l in aulBesigiFilter.sort(
                        ...
                        <div class="bg-[#252a36] relative cell-center h-6 w-full flex items-center justify-center">
                            <div class="absolute top-0 left-0 h-full rounded w-full" :style="{
                                background: getColorFromGradient(
                                    l.percentage_change ?? 0
                                ),
                                width: `${l.percentage_change > 100
                                    ? 100
                                    : l.percentage_change ?? 0
                                    }%`,
                            }"></div>
                            <p class="absolute text-[12px]">
                                {{ `${Numeral(l.percentage_change > 100 ? 100 : l.percentage_change ?? 0)}%` }}
                            </p>
                        </div>
                    </div>
                </div>
