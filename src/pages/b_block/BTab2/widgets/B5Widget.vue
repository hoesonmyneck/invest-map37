        ...
        <div class="absolute pl-4 z-[2000] overflow-auto w-full">
            <ul class="py-1 flex gap-2 text-nowrap">
                <template v-for="l in listLabels" :key="l.name">
                    <li @click="programStore.setCurrentKey(l.key)"
                        class="w-max p-1 bg-[#252A36] text-xs cursor-pointer items-center gap-2 border justify-between mb-1 px-3 h-max"
                        :class="{ 'border-[#3090E8]': current_key === l.key, 'border-gray-700': current_key !== l.key }">
        ...
        <BaseMap :current-region="+currentRegion" :fill-color="(v) => {
            ...
            <div>
                ...
                <div class="flex items-center gap-2" v-if="groupByRegion">
                    <p class=" mb-2">Участники:
                        <span class="font-bold">{{
                            groupByRegion[slotProps.data.parent1_code].total }}</span>
                    </p>
                </div>
            </div>
...
const { current_key, currentRegion, serpin, aulAmanati, diplommenAulga } = storeToRefs(programStore);
...
const maxTotal = computed(() => Object.values(groupByRegion.value).reduce((acc, curr) => acc + curr.total, 0));

const groupByRegion = computed(() => {
    if (current_key.value === 'serpin') return [...serpin.value].reduce((acc, curr) => {
    ...
    if (current_key.value === 'diplom') return diplommenAulga.value.reduce((acc, curr) => {
        if (acc[curr.parent1_code]) {
    ...
    if (current_key.value === 'auyl_amanat') return aulAmanati.value.reduce((acc, curr) => {
        if (acc[curr.parent1_code]) {
            const item = acc[curr.parent1_code]
